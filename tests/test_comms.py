...
def test_comms_sign_small_tx(yield_dongle):
    ...
    with yield_dongle() as dongle:
        ...
        encoded_tx = rlp.encode(tx, RLPTx)
        ...
        vrsbytes = dongle_send_data(dongle, SIGN_TX_FIRST_DATA, payload, Lc=int(len(payload)).to_bytes(1, 'big'))
...
def test_comms_sign_large_tx(yield_dongle):
    ...
    with yield_dongle() as dongle:
        ...
        for chunk in chunks(payload):
            ...
            if chunk_count == 0:
                retval = dongle_send_data(dongle, 'SIGN_TX_FIRST_DATA', chunk, Lc=chunk_size.to_bytes(1, 'big'))
            else:
                retval = dongle_send_data(dongle, 'SIGN_TX_SECONDARY_DATA', chunk, Lc=chunk_size.to_bytes(1, 'big'))
            chunk_count += 1
